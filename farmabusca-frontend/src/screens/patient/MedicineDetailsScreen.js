...
const statusMap = {
  ...
  LOW_STOCK: ['Poucas unidades', '#92400E', colors.warningLight],
...
export default function MedicineDetailsScreen({ route, navigation }) {
  ...
  return <ScrollView style={styles.container} contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => load(true)} tintColor={colors.primary} />}>
    ...
    {loading ? <View style={styles.pad}><LoadingSkeleton rows={3} /></View> : error && !details ? <ErrorState message={error} onRetry={load} /> : <>
      ...
      <View style={styles.warning}><Ionicons name="warning-outline" size={22} color={colors.warning} /><Text style={styles.warningText}>Consulte um profissional de saúde antes de utilizar qualquer medicamento.</Text></View>
...
const styles = StyleSheet.create({
  ...
  warning: { flexDirection: 'row', gap: 10, marginHorizontal: spacing.xl, backgroundColor: colors.warningLight, padding: 14, borderRadius: radius.md },
  warningText: { ...typography.caption, color: '#78350F', flex: 1 },
